...
function eventsCars(cardPrincipal) {
    ...
    btnComprar.addEventListener('click', (e) => {
        ...
        deleteProductoCarrito(newProducto, precio);
...
function deleteProductoCarrito(carritoPrincipal, precio) {
    const btnDelete = carritoPrincipal.querySelector("#eliminar-carrito");

    btnDelete.addEventListener("click", () => {
        carritoPrincipal.remove();

        let valor = Number(precio.replace(/[^0-9.-]+/g,"")); 
        totalCarrito -= valor;
        document.getElementById("total-carrito").textContent = "$" + totalCarrito.toLocaleString();
    });
}
...
function actualizarTotal(precio) {
    // quitar el símbolo $ y comas si las hay
    let valor = Number(precio.replace(/[^0-9.-]+/g,"")); 
    totalCarrito += valor;
    document.getElementById("total-carrito").textContent = "$" + totalCarrito.toLocaleString();
}
